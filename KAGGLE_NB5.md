...
## Publish the implementation first (local Git Bash)

Run from the repository root. These commands intentionally exclude the report and
imagery notebook, which are unrelated untracked deliverables.

```bash
git add kaggle/kg_05c_balanced_tabular.py kaggle/kg_05d_nested_tabular.py tests/test_robust_evaluation.py tests/test_nested_evaluation.py KAGGLE_NB5.md
git commit -m "Fix nested country evaluation and add NB5 workflow"
